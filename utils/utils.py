...
#     img = torch.cat([item[0] for item in batch], dim=0)
#     omic = torch.cat([item[1] for item in batch],
...
def collate_MIL_survival_cluster(batch):
    img = torch.cat([item[0] for item in batch], dim=0)
    cluster_ids = torch.cat([item[1] for item in batch],
                            dim=0).type(torch.LongTensor)
    omic = torch.cat([item[2] for item in batch],
                     dim=0).type(torch.FloatTensor)
    ...
    for item in batch:
        if isinstance(item[0], dict):
            for key, val in item[0].items():
                if key not in omics:
                    ...
                    img[key].append(val)
            for key in img:
                img[key] = torch.stack(img[key], dim=0)
        else:
        ...
        for key, val in item[1].items():
            if key not in omics:
                ...
                omics[key].append(val)
    for key in omics:
        omics[key] = torch.stack(omics[key], dim=0).type(torch.FloatTensor)
...
def get_split_loader(split_dataset, training=False, testing=False, weighted=False, mode='coattn', batch_size=1,collate=collate_MIL_survival):
    """
        ...
        if training:
            if weighted:
                weights = make_weights_for_balanced_classes_split(
                    split_dataset)
                loader = DataLoader(split_dataset, batch_size=batch_size, sampler=WeightedRandomSampler(
                    weights, len(weights)), collate_fn=collate,pin_memory=True, **kwargs)
            else:
                loader = DataLoader(split_dataset, batch_size=batch_size, sampler=RandomSampler(
                    split_dataset), collate_fn=collate,pin_memory=True, **kwargs)
        else:
            loader = DataLoader(split_dataset, batch_size=1, sampler=SequentialSampler(
                split_dataset), collate_fn=collate,pin_memory=True, **kwargs)

    else:
        ids = np.random.choice(np.arange(len(split_dataset), int(
            len(split_dataset)*0.1)), replace=False)
        loader = DataLoader(split_dataset, batch_size=1, sampler=SubsetSequentialSampler(
            ids), collate_fn=collate, **kwargs)
...
def get_optim(model, args):
    if args.opt == "adam":
        optimizer = optim.Adam(filter(
            lambda p: p.requires_grad, model.parameters()), lr=args.lr, weight_decay=args.reg)
    elif args.opt == 'sgd':
...
class CrossEntropySurvLoss(object):
    ...
    def __call__(self, h, y, c, t, S=None, alpha=None):
        if alpha is None:
            return ce_loss(h, S, y, c, alpha=self.alpha)
        else:
            return ce_loss(h, S, y, c, alpha=alpha)
...
def get_custom_exp_code(args):
    ...
    if args.model_type == 'porpoise_mmf':
        ...
        param_code += 'SnnMIL'
    elif args.model_type in ['idea1', 'idea2']:
        param_code += args.model_type
        if args.fuse != "None":
            param_code += '+'+args.fuse
        if args.use_transformer:
            param_code += '+transformer'
        param_code += '+'+args.mil_method
    elif args.model_type == 'gt':
        param_code += args.model_type
        if args.fuse != "None":
            param_code += '+'+args.fuse
        param_code += '+l'+str(args.num_layer)
    elif args.model_type in ['wsi-cluster']:
        param_code += args.model_type
        param_code += '+'+args.cluster_agg_method
        param_code += '+'+args.cluster_pool_method
        param_code += '+'+args.phenotype_pool_method
        param_code += '+'+str(args.num_cluster)
    elif args.model_type in ['idea2-2','idea2-2SnnMIL']:
        param_code += args.model_type
        param_code += '+'+args.cluster_agg_method
        param_code += '+'+args.cluster_pool_method
        param_code += '+'+args.phenotype_pool_method
        param_code += '+'+str(args.num_cluster)
        param_code += '+'+args.cluster_sim_fun
        param_code += '+maxiter'+str(args.cluster_max_iter)
        param_code += '+clusterlr'+str(args.cluster_lr)
        param_code += '+'+str(args.filter_start_epoch)
        param_code += '+'+str(args.filter_prob)
        param_code += '+'+str(args.global_centers_update_method)
    else:
        param_code += args.model_type

    # gene type
    if args.target_gene != None:
        if args.target_gene == 'mrna':
            param_code += '_mrna'
        elif args.target_gene == 'mrna_rnaseq':
            param_code += '_mrna-rnaseq'
        elif args.target_gene == 'signatures':
            param_code += '_signatures'
        elif args.target_gene == 'signatures_rnaseq':
            param_code += '_signatures-rnaseq'
        elif args.target_gene == 'signatures_rnaseq_cnv':
            param_code += '_signatures-rnaseq-cnv'
        else:
            raise NotImplementedError
    ...
    if args.reg_type != 'None':
        param_code += '_%sreg%s' % (args.reg_type,
                                    format(args.lambda_reg, '.0e'))

    if args.dropinput:
        param_code += '_drop%s' % str(int(args.dropinput*100))

    param_code += '_%s' % args.study
    
    if args.drop_instance!=0:
        param_code+='_dropinstance'+str(args.drop_instance)
